...
>
    ...
    @if ($extraHeadingColumn || $groupsOnly)
        ...
        >
            <span class="fi-ta-summary-row-heading {{-- px-3 py-4 --}} px-3 py-1">
                {{ $heading }}
            </span>
        </x-filament-tables::cell>
    @else
        ...
        @if (($loop->first || $extraHeadingColumn || $groupsOnly || ($loop->iteration > $headingColumnSpan)) && ($placeholderColumns || $column->hasSummary()))
            ...
            >
                @if ($loop->first && (! $extraHeadingColumn) && (! $groupsOnly))
                    <span
                        class="fi-ta-summary-row-heading flex {{-- px-3 py-4 --}} px-3 py-1 text-sm font-medium text-gray-950 dark:text-white"
                    >
                        {{ $heading }}
                    </span>
